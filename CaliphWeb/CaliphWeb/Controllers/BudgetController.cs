...
namespace CaliphWeb.Controllers
{
    ...
    public class BudgetController : Controller
    {
        ...
        public BudgetController(IMasterDataService masterService, ICaliphAPIHelper caliphAPIHelper, IUserService userService)
        {
        ...
        public async Task<ActionResult> Index2()
        {
            ...
            model.Income.IncomeData = response.Data.Where(p => p.BudgetYear == DateTime.Now.Year).FirstOrDefault();
        ...
        //refresh propotion
        [HttpPost]
        public async Task<ActionResult> RefreshPropotionTab(int incomeYear)
        {
            BudgetFilter filter = new BudgetFilter() { UserId = UserHelper.GetLoginUserViewModel().UserId, BudgetYear = incomeYear };
            BudgetModel model = new BudgetModel();
            var response = await _caliphAPIHelper.PostAsync<BudgetFilter, ResponseData<List<BudgetView>>>(filter, "/api/v1/budget/simulator-get-by-filter");
